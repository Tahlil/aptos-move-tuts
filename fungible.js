...
class AdminClient extends Provider {
  ...
  async mintCoin(admin, receiverAddress, amount) {
    const rawTxn = await this.generateTransaction(admin.address(), {
  ...
  async burnCoin(admin, fromAddress, amount) {
    const rawTxn = await this.generateTransaction(admin.address(), {
  ...
  async freeze(admin, targetAddress) {
    const rawTxn = await this.generateTransaction(admin.address(), {
  ...
  async unfreeze(admin, targetAddress){
    const rawTxn = await this.generateTransaction(admin.address(), {
  ...
  async getMetadata(admin) {
    const payload = {
      function: `${admin.address().hex()}::fa_coin::get_metadata`,
      type_arguments: [],
      arguments: [],
    };
    return ((await this.view(payload)))[0].inner;
  }
}
