...
(async () => {
  ...
  const privKeyInBytes = Uint8Array.from(Array.from(privateKey).map(letter => letter.charCodeAt(0)));
  const account1 = new aptos.AptosAccount(privKeyInBytes);
...
async function createCollection(collectionName, tokenName) {
    // Create the collection.
  // :!:>section_4
  const txnHash1 = await tokenClient.createCollection(
    account1,
    collectionName,
    "Tahlil's second collection",
    "https://static.wikia.nocookie.net/pokemonfanon/images/4/48/Swampert_9.jpg/revision/latest",
  ); // <:!:section_4
...
createCollection("Tahlil's Second collection", "Tahlil's token")
